...
if __name__ == "__main__":
    for task_arrival_rate in [0.01, 0.03 ,0.05, 0.07, 0.09, 0.11]:
        res_path = f"statistic/{node_num}/{task_arrival_rate}_{node_num}_{queue_len}_{co_num}_{time_limit}/greedy_res.csv"
        data_path = f"statistic/{node_num}/{task_arrival_rate}_{node_num}_{queue_len}_{co_num}_{time_limit}/greedy_data.csv"
        env, train_envs, test_envs = make_env(1, 1, 
                                            queue_len=queue_len,
                                            node_num=node_num,
                                            task_arrival_rate=task_arrival_rate,
                                            state_dim=2,max_draw_steps=100,
                                            co_num=co_num)
        env.evaluate=True
